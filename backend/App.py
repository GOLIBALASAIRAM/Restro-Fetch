...
def search_restaurants():
    ...
    country_code = request.args.get('country', '')  # Now using country code
    average_spend = request.args.get('average_spend', '')
    cuisine = request.args.get('cuisine', '')
    latitude = request.args.get('latitude', type=float)
    longitude = request.args.get('longitude', type=float)
    radius = request.args.get('radius', 5)  # Default radius is 5 km if not provided
    
    print(f"Received Search Params: search_query={search_query}, country_code={country_code}, avg_spend={average_spend}, cuisine={cuisine}, latitude={latitude}, longitude={longitude}, radius={radius}")
    ...
    # Apply filters based on country code
    if country_code:
